...
class WallFollow(Node):
    ...
    def __init__(self):
        ...
        vesc_drive_topic = 'ackermann_drive' # used in real vehicle
...
#       self.get_logger().info("PID: error %4f, integral %4f, diff %4f,angle %4f." % (error, integral, error-prev_error, angle))
    ...
    def safe_controller(self, scan):
        ...
        angle = scan.angle_min - 5 * scan.angle_increment

        for i in range(0, len(scan.ranges)+1, 5):
            angle += scan.angle_increment*5
